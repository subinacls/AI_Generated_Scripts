...
namespace (optional): 
  the name of the OpenShift namespace to view configmaps in. 
  If not provided,
    the current project will be used.
configmap (optional): 
  the name of the specific configmap to view.
  If not provided,
    the user will be prompted to select one.

The function first checks if a specific configmap was provided. 
  If not, 
    it prompts the user to select one from a list of all configmaps in the specified namespace.
  Once a configmap has been selected,
    the function retrieves the configmap's data in JSON format using
        oc get configmap
        pipes the output to jq
        extracts the contents from the data section.
  Displays the results to the user through STDOUT
...
function view_configmap() {
  ...
  oc get configmap $configmap -n $namespace -o json | jq '.data ';
}; # view_configmap myproject myconfigmap
